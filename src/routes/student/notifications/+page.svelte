<script lang="ts">
	...
	let notifications = $state([...mockStudentNotifications]);
	...
	const approvedCount = $derived(notifications.filter((n) => n.type === 'request').length);
...
<div class="space-y-5">
	<div class="grid grid-cols-2 xl:grid-cols-4 gap-4">
		...
		<StatCard label="From Registrar" value={approvedCount} icon="fa-solid fa-file-lines" color="green" />
	...
	<div class="flex items-center justify-between gap-4 flex-wrap">
		...
		<div class="flex items-center gap-2">
			<div class="relative">
				...
				<input type="text" bind:value={search} placeholder="Search..." oninput={() => (currentPage = 1)} class="pl-8 pr-3 py-2 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-essu-blue/30 w-44" />
			</div>
			{#if unreadCount > 0}
				<button onclick={markAllRead} class="px-3 py-2 text-sm border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50 transition-colors">
